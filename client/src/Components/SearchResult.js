...
const SearchResult = ({ src }) => {
  ...
  const handlePagination = (listings, pageNo) => {
    setCurrentListings(
      listings.data.slice(
        (pageNo - 1) * listingsPerPage,
        pageNo * listingsPerPage
      )
    );
  };
  ...
  const handleSearch = async (
    ...
    const recommendedListings = await getListingBySearchParameter(
      postalCode,
      rentPrice,
      rating,
      beds,
      ...
      <Box>
        ...
        <Box margin="auto" pt={3} pl={10}>
          ...
          >
            ...
            <SimpleGrid columns={{ base: 1, md: 4 }} spacing={10} mt={2}>
